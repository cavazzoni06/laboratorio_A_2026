 ...
 */
 public int calcolaPostiPrenotati(Proiezione proiezione) {
    int totale = 0;
    for (Prenotazione p : prenotazioni) {
        if (p.getProiezione().getTitolo().equals(proiezione.getTitolo()) && p.getProiezione().getDataOra().equals(proiezione.getDataOra())) {
 ...
 */
public Prenotazione creaPrenotazione(String usernameCliente,Proiezione proiezione,int numeroBiglietti) {
...
if (!proiezione.getDataOra().isAfter(adesso)) {
    ...
    Prenotazione nuovaPrenotazione =
        new Prenotazione(codice, usernameCliente, proiezione, numeroBiglietti);
 ...
 */
public void caricaPrenotazioniDaFile(String percorso, List<Proiezione> proiezioniDisponibili) throws IOException {
