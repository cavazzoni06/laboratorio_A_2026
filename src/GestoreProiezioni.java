...
public class GestoreProiezioni {
        ...
        public void caricaProiezioniDaFile(String percorso) throws IOException {
            ...
            while ((riga = reader.readLine()) != null) {
                ...
                Proiezione proiezione = new Proiezione(
                        dataOra, titolo, genere, regista, anno, durataMinuti,
                        etaMinima, prezzoBiglietto);
    ...
    // CERCA PROIEZIONE
        ...
        public List<Proiezione> cercaProiezione(String titolo, String genere,
                        LocalDateTime dataInizio, LocalDateTime dataFine, Double costoMassimo) {
        ...
        for (Proiezione p : proiezioni) {
            ...
            if (genere != null && !genere.isEmpty()) {
                ...
                if (!p.getGenere().equalsIgnoreCase(genere)) {
                    trovato = false;
                }
            }
    ...
    // VISUALIZZA PROIEZIONE
        ...
        for (Prenotazione prenotazione : prenotazioni) {
            if (prenotazione.getProiezione() == p) {
                postiPrenotati += prenotazione.getNumeroBiglietti();
            }
        }
        ...
        System.out.println("Durata: " + p.getDurataMinuti() + " minuti");
        System.out.println("Età minima: " + p.getEtaMinima());
    ...
    // AGGIUNGI PROIEZIONE
        ...
        LocalDateTime nuovaFine = nuovaInizio.plusMinutes(nuovaProiezione.getDurataMinuti());

        for (Proiezione p : proiezioni) {
            LocalDateTime inizio = p.getDataOra();
            LocalDateTime fine = inizio.plusMinutes(p.getDurataMinuti());
    ...
    // MODIFICA PROIEZIONE
        ...
        for (Prenotazione prenotazione : prenotazioni) {
            if (prenotazione.getProiezione() == proiezione) {
                return false;
            }
        }

        // Controlla sovrapposizione
        LocalDateTime nuovoInizio = nuovaProiezione.getDataOra();
        LocalDateTime nuovaFine = nuovoInizio.plusMinutes(nuovaProiezione.getDurataMinuti());
        ...
        // Modifica i dati
        proiezione.setDataOra(nuovaProiezione.getDataOra());
        proiezione.setTitolo(nuovaProiezione.getTitolo());
        proiezione.setGenere(nuovaProiezione.getGenere());
        proiezione.setRegista(nuovaProiezione.getRegista());
        proiezione.setAnno(nuovaProiezione.getAnno());
        proiezione.setDurataMinuti(nuovaProiezione.getDurataMinuti());
        proiezione.setEtaMinima(nuovaProiezione.getEtaMinima());
        proiezione.setPrezzoBiglietto(nuovaProiezione.getPrezzoBiglietto());
